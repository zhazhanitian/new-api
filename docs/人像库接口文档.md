...
```text
https://{你的平台域名}/v1/portrait/...
```

下文示例统一使用占位符：`https://api.example.com`
...
```bash
curl -X POST 'https://api.example.com/v1/portrait/groups' \
...
```bash
curl 'https://api.example.com/v1/portrait/groups?p=1&page_size=20' \
  -H 'Authorization: Bearer sk-your-token'
```
...
```bash
curl -X PUT 'https://api.example.com/v1/portrait/groups/ag-7f3c2b1a9e8d4c6f' \
  -H 'Authorization: Bearer sk-your-token' \
  -H 'Content-Type: application/json' \
  -d '{
    "name": "Seedance-主角素材组",
...
```bash
curl -X POST 'https://api.example.com/v1/portrait/assets' \
  -H 'Authorization: Bearer sk-your-token' \
  -H 'Content-Type: application/json' \
  -d '{
    "group_id": "ag-7f3c2b1a9e8d4c6f",
...
```bash
# 全部素材（第 1 页，每页 20 条）
curl 'https://api.example.com/v1/portrait/assets?p=1&page_size=20' \
  -H 'Authorization: Bearer sk-your-token'

# 按素材组过滤
curl 'https://api.example.com/v1/portrait/assets?group_id=ag-7f3c2b1a9e8d4c6f&p=1&page_size=10' \
  -H 'Authorization: Bearer sk-your-token'
```
...
```bash
curl 'https://api.example.com/v1/portrait/assets/asset-20260520181205-vvds9' \
  -H 'Authorization: Bearer sk-your-token'
```
...
```bash
curl -X PUT 'https://api.example.com/v1/portrait/assets/asset-20260520181205-vvds9' \
  -H 'Authorization: Bearer sk-your-token' \
  -H 'Content-Type: application/json' \
  -d '{ "name": "主角正面照（修正版）" }'
```
